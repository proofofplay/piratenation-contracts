...
uint256 constant ID = uint256(
    keccak256("game.piratenation.parentcomponent.v1")
);
...
library ParentComponentStorage {
 ...
 * @dev Creates a child-parent relationship between an entity and a parent entity
 */
contract ParentComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Parent entity that this entity is a child of. If 0, then this entity is a root entity
        keys[0] = "parent_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param parentEntity Parent entity that this entity is a child of. If 0, then this entity is a root entity
     */
    function setValue(
        uint256 entity,
        uint256 parentEntity
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(parentEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = ParentComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return parentEntity Parent entity that this entity is a child of. If 0, then this entity is a root entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 parentEntity) {
        if (has(entity)) {
            Layout memory s = ParentComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (parentEntity) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ParentComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.parentEntity);
    }
    ...
    function setBytes(
        ...
        (s.parentEntity) = abi.decode(value, (uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.parentEntity) = abi.decode(values[i], (uint256));
    ...
    function remove(
        ...
        delete ParentComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ParentComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.parentEntity = value.parentEntity;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.parentEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.parentEntity);
    }
}
