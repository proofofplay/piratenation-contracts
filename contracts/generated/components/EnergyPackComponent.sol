...
uint256 constant ID = uint256(
    keccak256("game.piratenation.energypackcomponent.v1")
);
...
library EnergyPackComponentStorage {
...
/**
 * @title EnergyPackComponent
 * @dev Define the energy amount and loot cost for an energy pack
 */
contract EnergyPackComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The energy amount to grant
        keys[0] = "energy_amount";
     ...
     *
     * @param entity Entity to get value for
     * @param energyAmount The energy amount to grant
    ...
    function setValue(
        ...
        _setValue(entity, Layout(energyAmount, lootEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = EnergyPackComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return energyAmount The energy amount to grant
     * @return lootEntity The loot entity pointing to the price of the pack
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 energyAmount, uint256 lootEntity) {
        if (has(entity)) {
            Layout memory s = EnergyPackComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (energyAmount, lootEntity) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = EnergyPackComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.energyAmount);
    ...
    function getBytes(
        ...
        Layout memory s = EnergyPackComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.energyAmount, s.lootEntity) = abi.decode(value, (uint256, uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.energyAmount, s.lootEntity) = abi.decode(
                values[i],
                (uint256, uint256)
            );
    ...
    function remove(
        ...
        delete EnergyPackComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete EnergyPackComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.energyAmount = value.energyAmount;
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.energyAmount, value.lootEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.energyAmount, value.lootEntity);
    }
}
