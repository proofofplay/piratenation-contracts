...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gemformulacomponent.v1")
);
...
library GemFormulaComponentStorage {
...
/**
 * @title GemFormulaComponent
 * @dev Define the piecewise formula for gem calculation
 */
contract GemFormulaComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param numerator Numerator variable of formula
     * @param denominator Denominator variable of formula
     * @param reduction Reduction variable of formula
     * @param offset Offset variable of formula
     */
    function setValue(
        uint256 entity,
        uint64 numerator,
        ...
        _setValue(entity, Layout(numerator, denominator, reduction, offset));
    }
    ...
    function getLayoutValue(
        ...
        value = GemFormulaComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return numerator Numerator variable of formula
     * @return denominator Denominator variable of formula
     * @return reduction Reduction variable of formula
     * @return offset Offset variable of formula
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = GemFormulaComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (numerator, denominator, reduction, offset) = abi.decode(
                _getEncodedValues(s),
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GemFormulaComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](4);
        values[0] = abi.encode(s.numerator);
        values[1] = abi.encode(s.denominator);
        values[2] = abi.encode(s.reduction);
        values[3] = abi.encode(s.offset);
    }
    ...
    function getBytes(
        ...
        Layout memory s = GemFormulaComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.numerator, s.denominator, s.reduction, s.offset) = abi.decode(
            value,
            (uint64, uint64, uint64, uint64)
        );
    ...
    function remove(
        ...
        delete GemFormulaComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete GemFormulaComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.numerator = value.numerator;
        s.denominator = value.denominator;
        s.reduction = value.reduction;
