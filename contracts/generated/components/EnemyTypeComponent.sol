...
uint256 constant ID = uint256(
    keccak256("game.piratenation.enemytypecomponent.v1")
);
...
library EnemyTypeComponentStorage {
...
/**
 * @title EnemyTypeComponent
 * @dev The enemy type of an entity
 */
contract EnemyTypeComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = EnemyTypeComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Type of the enemy
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (string memory value) {
        if (has(entity)) {
            Layout memory s = EnemyTypeComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = EnemyTypeComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = EnemyTypeComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete EnemyTypeComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete EnemyTypeComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
