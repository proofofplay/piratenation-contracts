...
uint256 constant ID = uint256(
    keccak256("game.piratenation.dicerollcomponent.v1")
);
...
library DiceRollComponentStorage {
...
/**
 * @title DiceRollComponent
 * @dev The dice roll of an entity
 */
contract DiceRollComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param roll1 First dice roll
     * @param roll2 Second dice roll
     */
    function setValue(
        uint256 entity,
        uint8 roll1,
        ...
        _setValue(entity, Layout(roll1, roll2));
    }
    ...
    function getLayoutValue(
        ...
        value = DiceRollComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return roll1 First dice roll
     * @return roll2 Second dice roll
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint8 roll1, uint8 roll2) {
        if (has(entity)) {
            Layout memory s = DiceRollComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (roll1, roll2) = abi.decode(_getEncodedValues(s), (uint8, uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = DiceRollComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.roll1);
        values[1] = abi.encode(s.roll2);
    }
    ...
    function getBytes(
        ...
        Layout memory s = DiceRollComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.roll1, s.roll2) = abi.decode(value, (uint8, uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.roll1, s.roll2) = abi.decode(values[i], (uint8, uint8));
    ...
    function remove(
        ...
        delete DiceRollComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete DiceRollComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.roll1, value.roll2));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.roll1, value.roll2);
    }
}
