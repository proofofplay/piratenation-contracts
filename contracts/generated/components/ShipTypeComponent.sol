...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shiptypecomponent.v1")
);

struct Layout {
    uint128 shipType;
    uint128 shipBaseType;
}

library ShipTypeComponentStorage {
...
/**
 * @title ShipTypeComponent
 * @dev Define the type of a ship and its base type
 */
contract ShipTypeComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The base type of the ship
        keys[1] = "ship_base_type";
     ...
     * @param shipBaseType The base type of the ship
     */
    function setValue(
        uint256 entity,
        uint128 shipType,
        ...
        _setValue(entity, Layout(shipType, shipBaseType));
    }
    ...
    function getLayoutValue(
        ...
        value = ShipTypeComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return shipType The type of the ship
     * @return shipBaseType The base type of the ship
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint128 shipType, uint128 shipBaseType) {
        if (has(entity)) {
            Layout memory s = ShipTypeComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (shipType, shipBaseType) = abi.decode(
                _getEncodedValues(s),
                (uint128, uint128)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShipTypeComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.shipType);
        values[1] = abi.encode(s.shipBaseType);
    }
    ...
    function getBytes(
        ...
        Layout memory s = ShipTypeComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.shipType, s.shipBaseType) = abi.decode(value, (uint128, uint128));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.shipType, s.shipBaseType) = abi.decode(
                values[i],
                (uint128, uint128)
            );
    ...
    function remove(
        ...
        delete ShipTypeComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ShipTypeComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.shipType = value.shipType;
        s.shipBaseType = value.shipBaseType;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.shipType, value.shipBaseType));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.shipType, value.shipBaseType);
    }
}
