...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shoptokenpricingcomponent.v1")
);
...
library ShopTokenPricingComponentStorage {
...
/**
 * @title ShopTokenPricingComponent
 * @dev Pricing data for a shop SKU that&#39;s a custom ERC20 Token
 */
contract ShopTokenPricingComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Purchase price in ERC20 to its base value
        keys[0] = "price";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // Address of the Token to take as payment
        keys[1] = "token_address";
        values[1] = TypesLibrary.SchemaValue.ADDRESS;

        // ChainId of the ERC20 Token
        keys[2] = "chain_id";
     ...
     *
     * @param entity Entity to get value for
     * @param price Purchase price in ERC20 to its base value
     * @param tokenAddress Address of the Token to take as payment
     * @param chainId ChainId of the ERC20 Token
     */
    function setValue(
        uint256 entity,
        uint256 price,
        ...
        _setValue(entity, Layout(price, tokenAddress, chainId));
    }
    ...
    function getLayoutValue(
        ...
        value = ShopTokenPricingComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return price Purchase price in ERC20 to its base value
     * @return tokenAddress Address of the Token to take as payment
     * @return chainId ChainId of the ERC20 Token
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 price, address tokenAddress, uint32 chainId)
    {
        if (has(entity)) {
            Layout memory s = ShopTokenPricingComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (price, tokenAddress, chainId) = abi.decode(
                _getEncodedValues(s),
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShopTokenPricingComponentStorage
            .layout()
            .entityIdToStruct[entity];
        ...
        values[1] = abi.encode(s.tokenAddress);
        values[2] = abi.encode(s.chainId);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ShopTokenPricingComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.price, s.tokenAddress, s.chainId) = abi.decode(
            value,
            (uint256, address, uint32)
        );
    ...
    function remove(
        ...
        delete ShopTokenPricingComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.price, value.tokenAddress, value.chainId)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.price, value.tokenAddress, value.chainId);
    }
}
