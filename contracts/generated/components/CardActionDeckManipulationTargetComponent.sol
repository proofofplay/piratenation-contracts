...
uint256 constant ID = uint256(
    keccak256("game.piratenation.cardactiondeckmanipulationtargetcomponent.v1")
);
...
library CardActionDeckManipulationTargetComponentStorage {
...
/**
 * @title CardActionDeckManipulationTargetComponent
 * @dev Card Action Deck Manipulation Target Component is a card action component that defines data for where the certain deck actions are targetted to
 */
contract CardActionDeckManipulationTargetComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Card pile this action is targetted to - discard, draw, hand
     ...
     *
     * @param entity Entity to get value for
     * @param cardPileType Card pile this action is targetted to - discard, draw, hand
     */
    function setValue(
        uint256 entity,
        uint8 cardPileType
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(cardPileType));
    }
    ...
    function getLayoutValue(
        ...
        value = CardActionDeckManipulationTargetComponentStorage
            .layout()
     ...
     *
     * @param entity Entity to get value for
     * @return cardPileType Card pile this action is targetted to - discard, draw, hand
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint8 cardPileType) {
        if (has(entity)) {
            Layout memory s = CardActionDeckManipulationTargetComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (cardPileType) = abi.decode(_getEncodedValues(s), (uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CardActionDeckManipulationTargetComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = CardActionDeckManipulationTargetComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.cardPileType) = abi.decode(value, (uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.cardPileType) = abi.decode(values[i], (uint8));
    ...
    function remove(
        ...
        delete CardActionDeckManipulationTargetComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.cardPileType));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.cardPileType);
    }
}
