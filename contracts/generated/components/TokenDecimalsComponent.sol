...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tokendecimalcomponent.v1")
);
...
library TokenDecimalsComponentStorage {
...
/**
 * @title TokenDecimalsComponent
 * @dev A component to set the value to display as a decimal
 */
contract TokenDecimalsComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The number of decimal places value
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value The number of decimal places value
     */
    function setValue(
        uint256 entity,
        uint8 value
    ...
    function getLayoutValue(
        ...
        value = TokenDecimalsComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value The number of decimal places value
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint8 value) {
        if (has(entity)) {
            Layout memory s = TokenDecimalsComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (value) = abi.decode(_getEncodedValues(s), (uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TokenDecimalsComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = TokenDecimalsComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.value) = abi.decode(value, (uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.value) = abi.decode(values[i], (uint8));
    ...
    function remove(
        ...
        delete TokenDecimalsComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TokenDecimalsComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
