...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gemtransformeligiblecomponent.v1")
);
...
library GemTransformEligibleComponentStorage {
...
/**
 * @title GemTransformEligibleComponent
 * @dev Define if a transform is eligible for gems
 */
contract GemTransformEligibleComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether the transform is eligible for gems or not (default is false)
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether the transform is eligible for gems or not (default is false)
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = GemTransformEligibleComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether the transform is eligible for gems or not (default is false)
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = GemTransformEligibleComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GemTransformEligibleComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete GemTransformEligibleComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete GemTransformEligibleComponentStorage
                .layout()
                .entityIdToStruct[entities[i]];
        }
