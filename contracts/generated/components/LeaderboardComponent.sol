...
uint256 constant ID = uint256(
    keccak256("game.piratenation.leaderboardcomponent.v1")
);
...
library LeaderboardComponentStorage {
...
/**
 * @title LeaderboardComponent
 * @dev The configuration for a leaderboard
 */
contract LeaderboardComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The item entity that the leaderboard uses to determine progress
        keys[0] = "item_entity";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // The popup entity to display when the leaderboard details are viewed
        keys[1] = "popup_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param itemEntity The item entity that the leaderboard uses to determine progress
     * @param popupEntity The popup entity to display when the leaderboard details are viewed
     * @param version The version of the leaderboard
     */
    function setValue(
        uint256 entity,
        uint256 itemEntity,
        ...
        _setValue(entity, Layout(itemEntity, popupEntity, version));
    }
    ...
    function getLayoutValue(
        ...
        value = LeaderboardComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return itemEntity The item entity that the leaderboard uses to determine progress
     * @return popupEntity The popup entity to display when the leaderboard details are viewed
     * @return version The version of the leaderboard
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 itemEntity, uint256 popupEntity, uint256 version)
    {
        if (has(entity)) {
            Layout memory s = LeaderboardComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (itemEntity, popupEntity, version) = abi.decode(
                _getEncodedValues(s),
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = LeaderboardComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](3);
        values[0] = abi.encode(s.itemEntity);
        values[1] = abi.encode(s.popupEntity);
        values[2] = abi.encode(s.version);
    }
    ...
    function getBytes(
        ...
        Layout memory s = LeaderboardComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.itemEntity, s.popupEntity, s.version) = abi.decode(
            value,
            (uint256, uint256, uint256)
        );
    ...
    function remove(
        ...
        delete LeaderboardComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete LeaderboardComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.itemEntity = value.itemEntity;
        s.popupEntity = value.popupEntity;
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.itemEntity, value.popupEntity, value.version)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.itemEntity, value.popupEntity, value.version);
    }
}
