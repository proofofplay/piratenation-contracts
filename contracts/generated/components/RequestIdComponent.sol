...
uint256 constant ID = uint256(
    keccak256("game.piratenation.requestidcomponent.v1")
);
...
library RequestIdComponentStorage {
...
/**
 * @title RequestIdComponent
 * @dev Component for tracking a request entity
 */
contract RequestIdComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = RequestIdComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value request entity id
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = RequestIdComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = RequestIdComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = RequestIdComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete RequestIdComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete RequestIdComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
