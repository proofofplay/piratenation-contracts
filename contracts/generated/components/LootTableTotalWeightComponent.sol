...
uint256 constant ID = uint256(
    keccak256("game.piratenation.loottabletotalweightcomponent.v1")
);
...
library LootTableTotalWeightComponentStorage {
...
/**
 * @title LootTableTotalWeightComponent
 * @dev Total weight of all rows in the table
 */
contract LootTableTotalWeightComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(totalWeight));
    }
    ...
    function getLayoutValue(
        ...
        value = LootTableTotalWeightComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 totalWeight) {
        if (has(entity)) {
            Layout memory s = LootTableTotalWeightComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (totalWeight) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = LootTableTotalWeightComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.totalWeight);
    }
    ...
    function setBytes(
        ...
        Layout memory s = LootTableTotalWeightComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.totalWeight) = abi.decode(value, (uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.totalWeight) = abi.decode(values[i], (uint256));
    ...
    function remove(
        ...
        delete LootTableTotalWeightComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete LootTableTotalWeightComponentStorage
                .layout()
                .entityIdToStruct[entities[i]];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.totalWeight));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.totalWeight);
    }
}
