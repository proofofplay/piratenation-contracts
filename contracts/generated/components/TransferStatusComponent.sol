...
uint256 constant ID = uint256(
    keccak256("game.piratenation.transferstatuscomponent.v1")
);
...
library TransferStatusComponentStorage {
...
/**
 * @title TransferStatusComponent
 * @dev Describes a transfer between accounts
 */
contract TransferStatusComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(to, from, status));
    }
    ...
    function getLayoutValue(
        ...
        value = TransferStatusComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return to To wallet address
     * @return from From wallet address
     * @return status Status of the transfer
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (address to, address from, uint8 status) {
        if (has(entity)) {
            Layout memory s = TransferStatusComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (to, from, status) = abi.decode(
                _getEncodedValues(s),
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TransferStatusComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](3);
        values[0] = abi.encode(s.to);
        values[1] = abi.encode(s.from);
        values[2] = abi.encode(s.status);
    }
    ...
    function setBytes(
        ...
        Layout memory s = TransferStatusComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.to, s.from, s.status) = abi.decode(value, (address, address, uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.to, s.from, s.status) = abi.decode(
                values[i],
                (address, address, uint8)
            );
    ...
    function remove(
        ...
        delete TransferStatusComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TransferStatusComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.to, value.from, value.status));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.to, value.from, value.status);
    }
}
