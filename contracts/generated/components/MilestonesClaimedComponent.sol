...
uint256 constant ID = uint256(
    keccak256("game.piratenation.milestonesclaimedcomponent")
);

struct Layout {
    uint256[] milestoneIndexesClaimed;
}

library MilestonesClaimedComponentStorage {
...
/**
 * @title MilestonesClaimedComponent
 * @dev Component for storing the claimed progress of a set of milestones for an entity
 */
contract MilestonesClaimedComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The indexes of the milestones claimed
        keys[0] = "milestone_indexes_claimed";
     ...
     *
     * @param entity Entity to get value for
     * @param milestoneIndexesClaimed The indexes of the milestones claimed
     */
    function setValue(
        uint256 entity,
        uint256[] memory milestoneIndexesClaimed
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(milestoneIndexesClaimed));
    }
    ...
    function append(
        ...
        Layout storage s = MilestonesClaimedComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.milestoneIndexesClaimed.length; i++) {
            s.milestoneIndexesClaimed.push(values.milestoneIndexesClaimed[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.milestoneIndexesClaimed));
    }
    ...
    function removeValueAtIndex(
        ...
        // Get the last index
        uint256 lastIndexInArray = s.milestoneIndexesClaimed.length - 1;

        // Move the last value to the index to pop
        if (index != lastIndexInArray) {
            s.milestoneIndexesClaimed[index] = s.milestoneIndexesClaimed[
                lastIndexInArray
            ];
        }

        // Pop the last value
        s.milestoneIndexesClaimed.pop();
    ...
    function getLayoutValue(
        ...
        value = MilestonesClaimedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return milestoneIndexesClaimed The indexes of the milestones claimed
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory milestoneIndexesClaimed) {
        if (has(entity)) {
            Layout memory s = MilestonesClaimedComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (milestoneIndexesClaimed) = abi.decode(
                _getEncodedValues(s),
                (uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.milestoneIndexesClaimed);
    }
    ...
    function setBytes(
        ...
        Layout memory s = MilestonesClaimedComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.milestoneIndexesClaimed) = abi.decode(value, (uint256[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.milestoneIndexesClaimed) = abi.decode(values[i], (uint256[]));
    ...
    function remove(
        ...
        delete MilestonesClaimedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.milestoneIndexesClaimed = value.milestoneIndexesClaimed;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.milestoneIndexesClaimed));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.milestoneIndexesClaimed);
    }
}
