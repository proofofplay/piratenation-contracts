...
contract TransformRefundComponent is BaseStorageComponentV2 {
    ...
    function removeValueAtIndex(
        ...
        if (index != lastIndexInArray) {
            s.successRefundProbability[index] = s.successRefundProbability[
                lastIndexInArray
            ];
            s.failureRefundProbability[index] = s.failureRefundProbability[
                lastIndexInArray
            ];
        }
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.successRefundProbability, s.failureRefundProbability) = abi
                .decode(values[i], (uint32[], uint32[]));
