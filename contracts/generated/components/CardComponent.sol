...
uint256 constant ID = uint256(keccak256("game.piratenation.cardcomponent.v1"));

struct Layout {
    uint8 cardActionType;
    uint8 pointCost;
    uint256[] actionEntities;
}

library CardComponentStorage {
...
/**
 * @title CardComponent
 * @dev Card Component is a component that defines data for combat cards and references to actions
 */
contract CardComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Card Action Type as defined: damage, manipulation, etc
        ...
        // Card actions entity ids that are used as part of the card
        keys[2] = "action_entities";
    ...
    function setValue(
        ...
        uint8 cardActionType,
        uint8 pointCost,
        uint256[] memory actionEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(cardActionType, pointCost, actionEntities));
    }
    ...
    function getLayoutValue(
        ...
        value = CardComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    {
        if (has(entity)) {
            Layout memory s = CardComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (cardActionType, pointCost, actionEntities) = abi.decode(
                _getEncodedValues(s),
                (uint8, uint8, uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CardComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](3);
        values[0] = abi.encode(s.cardActionType);
        values[1] = abi.encode(s.pointCost);
        values[2] = abi.encode(s.actionEntities);
    }
    ...
    function setBytes(
        ...
        (s.cardActionType, s.pointCost, s.actionEntities) = abi.decode(
            value,
            (uint8, uint8, uint256[])
        );
    ...
    function remove(
        ...
        delete CardComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete CardComponentStorage.layout().entityIdToStruct[entities[i]];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.cardActionType = value.cardActionType;
        s.pointCost = value.pointCost;
        s.actionEntities = value.actionEntities;
    }
