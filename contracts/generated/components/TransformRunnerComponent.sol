...
uint256 constant ID = uint256(
    keccak256("game.piratenation.transformrunnercomponent.v1")
);

struct Layout {
    uint256[] transformRunnerEntities;
}

library TransformRunnerComponentStorage {
...
/**
 * @title TransformRunnerComponent
 * @dev This component is used to define the runners for a given transform
 */
contract TransformRunnerComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entities of the transform runners to use for this transform
        keys[0] = "transform_runner_entities";
     ...
     *
     * @param entity Entity to get value for
     * @param transformRunnerEntities Entities of the transform runners to use for this transform
     */
    function setValue(
        uint256 entity,
        uint256[] memory transformRunnerEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(transformRunnerEntities));
    }
    ...
    function append(
        ...
        Layout storage s = TransformRunnerComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.transformRunnerEntities.length; i++) {
            s.transformRunnerEntities.push(values.transformRunnerEntities[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.transformRunnerEntities));
    }
    ...
    function removeValueAtIndex(
        ...
        // Get the last index
        uint256 lastIndexInArray = s.transformRunnerEntities.length - 1;

        // Move the last value to the index to pop
        if (index != lastIndexInArray) {
            s.transformRunnerEntities[index] = s.transformRunnerEntities[
                lastIndexInArray
            ];
        }

        // Pop the last value
        s.transformRunnerEntities.pop();
    ...
    function getLayoutValue(
        ...
        value = TransformRunnerComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return transformRunnerEntities Entities of the transform runners to use for this transform
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory transformRunnerEntities) {
        if (has(entity)) {
            Layout memory s = TransformRunnerComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (transformRunnerEntities) = abi.decode(
                _getEncodedValues(s),
                (uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.transformRunnerEntities);
    }
    ...
    function setBytes(
        ...
        Layout memory s = TransformRunnerComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.transformRunnerEntities) = abi.decode(value, (uint256[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.transformRunnerEntities) = abi.decode(values[i], (uint256[]));
    ...
    function remove(
        ...
        delete TransformRunnerComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.transformRunnerEntities = value.transformRunnerEntities;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.transformRunnerEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.transformRunnerEntities);
    }
}
