...
uint256 constant ID = uint256(
    keccak256("game.piratenation.upgradeableitemcomponent.v1")
);
...
library UpgradeableItemComponentStorage {
...
/**
 * @title UpgradeableItemComponent
 * @dev Define settings for an upgradeable entity
 */
contract UpgradeableItemComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Cost to upgrade to its next version
        keys[0] = "loot_entity";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // Entity ID of the next entity to upgrade to
        keys[1] = "next_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param lootEntity Cost to upgrade to its next version
     * @param nextEntity Entity ID of the next entity to upgrade to
     */
    function setValue(
        uint256 entity,
        uint256 lootEntity,
        ...
        _setValue(entity, Layout(lootEntity, nextEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = UpgradeableItemComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lootEntity Cost to upgrade to its next version
     * @return nextEntity Entity ID of the next entity to upgrade to
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 lootEntity, uint256 nextEntity) {
        if (has(entity)) {
            Layout memory s = UpgradeableItemComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (lootEntity, nextEntity) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = UpgradeableItemComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.lootEntity);
        values[1] = abi.encode(s.nextEntity);
    }
    ...
    function setBytes(
        ...
        Layout memory s = UpgradeableItemComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.lootEntity, s.nextEntity) = abi.decode(value, (uint256, uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.lootEntity, s.nextEntity) = abi.decode(
                values[i],
                (uint256, uint256)
            );
    ...
    function remove(
        ...
        delete UpgradeableItemComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.lootEntity, value.nextEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.lootEntity, value.nextEntity);
    }
}
