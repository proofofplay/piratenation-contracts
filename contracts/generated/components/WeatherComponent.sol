...
uint256 constant ID = uint256(
    keccak256("game.piratenation.weathercomponent.v1")
);
...
library WeatherComponentStorage {
 ...
 * @dev Weather of an entity
 */
contract WeatherComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = WeatherComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Weather type
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = WeatherComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = WeatherComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete WeatherComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete WeatherComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
