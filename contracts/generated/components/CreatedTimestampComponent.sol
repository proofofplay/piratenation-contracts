...
uint256 constant ID = uint256(
    keccak256("game.piratenation.createdtimestampcomponent.v1")
);
...
library CreatedTimestampComponentStorage {
...
/**
 * @title CreatedTimestampComponent
 * @dev A simple timestamp for creation time
 */
contract CreatedTimestampComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Creation time of the entity
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = CreatedTimestampComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Creation time of the entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = CreatedTimestampComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CreatedTimestampComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete CreatedTimestampComponentStorage.layout().entityIdToStruct[
            entity
        ];
