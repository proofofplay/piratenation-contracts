...
uint256 constant ID = uint256(
    keccak256("game.piratenation.hassoulcomponent.v1")
);
...
library HasSoulComponentStorage {
...
/**
 * @title HasSoulComponent
 * @dev The entity has a soul which allows transfers and overrides soulbound
 */
contract HasSoulComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether or not the entity has a soul
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether or not the entity has a soul
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = HasSoulComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether or not the entity has a soul
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = HasSoulComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = HasSoulComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete HasSoulComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete HasSoulComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
