...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gachalootcomponent.v2")
);
...
library GachaLootComponentStorage {
...
/**
 * @title GachaLootComponent
 * @dev Holds a single loot entity associated with a Gacha
 */
contract GachaLootComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The entity ID of the gacha
        keys[0] = "gacha_id";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // Output lootarraycomponent for this gacha
        keys[1] = "output_loot_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param gachaId The entity ID of the gacha
     * @param outputLootEntity Output lootarraycomponent for this gacha
     */
    function setValue(
        uint256 entity,
        uint256 gachaId,
        ...
        _setValue(entity, Layout(gachaId, outputLootEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = GachaLootComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return gachaId The entity ID of the gacha
     * @return outputLootEntity Output lootarraycomponent for this gacha
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 gachaId, uint256 outputLootEntity)
    {
        if (has(entity)) {
            Layout memory s = GachaLootComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (gachaId, outputLootEntity) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GachaLootComponentStorage.layout().entityIdToStruct[
            entity
        ];
        ...
        values[1] = abi.encode(s.outputLootEntity);
    }
    ...
    function getBytes(
        ...
        Layout memory s = GachaLootComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.gachaId, s.outputLootEntity) = abi.decode(value, (uint256, uint256));
    ...
    function remove(
        ...
        delete GachaLootComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete GachaLootComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.gachaId, value.outputLootEntity)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.gachaId, value.outputLootEntity);
    }
}
