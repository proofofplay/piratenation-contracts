...
uint256 constant ID = uint256(
    keccak256("game.piratenation.sagalivescomponent.v1")
);

struct Layout {
    uint256 lastLifeAmount;
    uint256 lastSpendTimestamp;
}

library SagaLivesComponentStorage {
...
/**
 * @title SagaLivesComponent
 * @dev Track a wallets lives in the Saga
 */
contract SagaLivesComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Saga lives amount at time of last spend
        keys[0] = "last_life_amount";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // Timestamp the life was last spent
     ...
     *
     * @param entity Entity to get value for
     * @param lastLifeAmount Saga lives amount at time of last spend
     * @param lastSpendTimestamp Timestamp the life was last spent
     */
    function setValue(
        uint256 entity,
        uint256 lastLifeAmount,
        ...
        _setValue(entity, Layout(lastLifeAmount, lastSpendTimestamp));
    }
    ...
    function getLayoutValue(
        ...
        value = SagaLivesComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lastLifeAmount Saga lives amount at time of last spend
     * @return lastSpendTimestamp Timestamp the life was last spent
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 lastLifeAmount, uint256 lastSpendTimestamp)
    {
        if (has(entity)) {
            Layout memory s = SagaLivesComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (lastLifeAmount, lastSpendTimestamp) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = SagaLivesComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.lastLifeAmount);
        values[1] = abi.encode(s.lastSpendTimestamp);
    }
    ...
    function getBytes(
        ...
        Layout memory s = SagaLivesComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.lastLifeAmount, s.lastSpendTimestamp) = abi.decode(
            value,
            (uint256, uint256)
        );
    ...
    function remove(
        ...
        delete SagaLivesComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete SagaLivesComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.lastLifeAmount = value.lastLifeAmount;
        s.lastSpendTimestamp = value.lastSpendTimestamp;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.lastLifeAmount, value.lastSpendTimestamp)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.lastLifeAmount, value.lastSpendTimestamp);
    }
}
