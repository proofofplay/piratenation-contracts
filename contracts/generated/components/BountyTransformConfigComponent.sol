...
uint256 constant ID = uint256(
    keccak256("game.piratenation.bountytransformconfigcomponent")
);
...
library BountyTransformConfigComponentStorage {
...
/**
 * @title BountyTransformConfigComponent
 * @dev This component defines rules for a Bounty transform.
 */
contract BountyTransformConfigComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Amount of XP to grant each pirate used in this bounty
        keys[0] = "success_xp";
     ...
     *
     * @param entity Entity to get value for
     * @param successXp Amount of XP to grant each pirate used in this bounty
     ...
     * @param timeLock Amount of time (in seconds) to complete this Bounty + NFTs are locked for
     */
    function setValue(
        uint256 entity,
        uint32 successXp,
        ...
        _setValue(entity, Layout(successXp, lowerBound, upperBound, timeLock));
    }
    ...
    function getLayoutValue(
        ...
        value = BountyTransformConfigComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return successXp Amount of XP to grant each pirate used in this bounty
     ...
     * @return timeLock Amount of time (in seconds) to complete this Bounty + NFTs are locked for
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = BountyTransformConfigComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (successXp, lowerBound, upperBound, timeLock) = abi.decode(
                _getEncodedValues(s),
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = BountyTransformConfigComponentStorage
            .layout()
            .entityIdToStruct[entity];
        ...
        values[3] = abi.encode(s.timeLock);
    }
    ...
    function setBytes(
        ...
        Layout memory s = BountyTransformConfigComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.successXp, s.lowerBound, s.upperBound, s.timeLock) = abi.decode(
            value,
            (uint32, uint32, uint32, uint32)
        );
    ...
    function remove(
        ...
        delete BountyTransformConfigComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete BountyTransformConfigComponentStorage
                .layout()
