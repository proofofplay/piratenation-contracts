...
uint256 constant ID = uint256(
    keccak256("game.piratenation.orderfulfilledreplaycomponent.v1")
);
...
library OrderFulfilledReplayComponentStorage {
...
/**
 * @title OrderFulfilledReplayComponent
 * @dev Track the timestamp of when an order has been fulfilled for replay protection
 */
contract OrderFulfilledReplayComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The timestamp of the order fulfillment
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value The timestamp of the order fulfillment
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = OrderFulfilledReplayComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value The timestamp of the order fulfillment
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = OrderFulfilledReplayComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = OrderFulfilledReplayComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete OrderFulfilledReplayComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete OrderFulfilledReplayComponentStorage
                .layout()
                .entityIdToStruct[entities[i]];
        }
