 ...
 */
contract SceneObjectGameItemComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(itemEntity));
    }
     ...
     *
     * @param entity Entity to get value for
     * @return itemEntity Entity ID of the item
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 itemEntity) {
        if (has(entity)) {
            Layout memory s = SceneObjectGameItemComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (itemEntity) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = SceneObjectGameItemComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = SceneObjectGameItemComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.itemEntity) = abi.decode(value, (uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.itemEntity) = abi.decode(values[i], (uint256));
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.itemEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.itemEntity);
    }
}
