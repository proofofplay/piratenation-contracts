...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shipwrightplacedcomponent.v1")
);
...
library ShipWrightPlacedComponentStorage {
...
/**
 * @title ShipWrightPlacedComponent
 * @dev Track a ShipWright placed on an island
 */
contract ShipWrightPlacedComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The owner of the shipwright instance
        keys[0] = "owner";
        values[0] = TypesLibrary.SchemaValue.ADDRESS;

        // The island the shipwright instance is placed on
        keys[1] = "island_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param owner The owner of the shipwright instance
     * @param islandEntity The island the shipwright instance is placed on
     */
    function setValue(
        uint256 entity,
        address owner,
        ...
        _setValue(entity, Layout(owner, islandEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = ShipWrightPlacedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return owner The owner of the shipwright instance
     * @return islandEntity The island the shipwright instance is placed on
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (address owner, uint256 islandEntity) {
        if (has(entity)) {
            Layout memory s = ShipWrightPlacedComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (owner, islandEntity) = abi.decode(
                _getEncodedValues(s),
                (address, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShipWrightPlacedComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.owner);
        values[1] = abi.encode(s.islandEntity);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ShipWrightPlacedComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.owner, s.islandEntity) = abi.decode(value, (address, uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.owner, s.islandEntity) = abi.decode(
                values[i],
                (address, uint256)
            );
    ...
    function remove(
        ...
        delete ShipWrightPlacedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.owner = value.owner;
        s.islandEntity = value.islandEntity;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.owner, value.islandEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.owner, value.islandEntity);
    }
}
