...
uint256 constant ID = uint256(
    keccak256("game.piratenation.sagalivestransformtrackercomponent.v1")
);
...
library SagaLivesTransformTrackerComponentStorage {
...
/**
 * @title SagaLivesTransformTrackerComponent
 * @dev Track data for a transform that deals with saga lives
 */
contract SagaLivesTransformTrackerComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ID of the saga transform that was run
        keys[0] = "transform_entity";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // Whether the saga transform was successful
        keys[1] = "success";
     ...
     *
     * @param entity Entity to get value for
     * @param transformEntity Entity ID of the saga transform that was run
     * @param success Whether the saga transform was successful
     */
    function setValue(
        uint256 entity,
        uint256 transformEntity,
        ...
        _setValue(entity, Layout(transformEntity, success));
    }
    ...
    function getLayoutValue(
        ...
        value = SagaLivesTransformTrackerComponentStorage
            .layout()
     ...
     *
     * @param entity Entity to get value for
     * @return transformEntity Entity ID of the saga transform that was run
     * @return success Whether the saga transform was successful
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 transformEntity, bool success) {
        if (has(entity)) {
            Layout memory s = SagaLivesTransformTrackerComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (transformEntity, success) = abi.decode(
                _getEncodedValues(s),
                (uint256, bool)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = SagaLivesTransformTrackerComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = SagaLivesTransformTrackerComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.transformEntity, s.success) = abi.decode(value, (uint256, bool));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.transformEntity, s.success) = abi.decode(
                values[i],
                (uint256, bool)
            );
    ...
    function remove(
        ...
        delete SagaLivesTransformTrackerComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.transformEntity, value.success));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.transformEntity, value.success);
    }
}
