...
uint256 constant ID = uint256(
    keccak256("game.piratenation.expiresatcomponent.v1")
);
...
library ExpiresAtComponentStorage {
...
/**
 * @title ExpiresAtComponent
 * @dev Track the timestamp an entity expires
 */
contract ExpiresAtComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The timestamp the entity expires
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value The timestamp the entity expires
     */
    function setValue(
        uint256 entity,
        uint32 value
    ...
    function getLayoutValue(
        ...
        value = ExpiresAtComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value The timestamp the entity expires
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint32 value) {
        if (has(entity)) {
            Layout memory s = ExpiresAtComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (value) = abi.decode(_getEncodedValues(s), (uint32));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ExpiresAtComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = ExpiresAtComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.value) = abi.decode(value, (uint32));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.value) = abi.decode(values[i], (uint32));
    ...
    function remove(
        ...
        delete ExpiresAtComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ExpiresAtComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
