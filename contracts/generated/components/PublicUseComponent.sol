...
uint256 constant ID = uint256(
    keccak256("game.piratenation.publicusecomponent.v1")
);
...
library PublicUseComponentStorage {
...
/**
 * @title PublicUseComponent
 * @dev Define if an entity is open for public or private use
 */
contract PublicUseComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether the entity is open for public use
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether the entity is open for public use
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = PublicUseComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether the entity is open for public use
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = PublicUseComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = PublicUseComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = PublicUseComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete PublicUseComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete PublicUseComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
