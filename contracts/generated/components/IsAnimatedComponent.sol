...
uint256 constant ID = uint256(
    keccak256("game.piratenation.isanimatedcomponent.v1")
);
...
library IsAnimatedComponentStorage {
...
/**
 * @title IsAnimatedComponent
 * @dev Whether an entity is animated
 */
contract IsAnimatedComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = IsAnimatedComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = IsAnimatedComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = IsAnimatedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = IsAnimatedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete IsAnimatedComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete IsAnimatedComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
