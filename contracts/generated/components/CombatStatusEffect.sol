...
uint256 constant ID = uint256(
    keccak256("game.piratenation.combatstatuseffect.v1")
);
...
library CombatStatusEffectStorage {
 ...
 */
contract CombatStatusEffect is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(
            entity,
            Layout(effectValue, durationValue, startCondition, statusEffectType)
        );
    }
    ...
    function getLayoutValue(
        ...
        value = CombatStatusEffectStorage.layout().entityIdToStruct[entity];
    }
    ...
    {
        if (has(entity)) {
            Layout memory s = CombatStatusEffectStorage
                .layout()
                .entityIdToStruct[entity];
            (effectValue, durationValue, startCondition, statusEffectType) = abi
                .decode(_getEncodedValues(s), (uint16, uint16, uint8, uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CombatStatusEffectStorage.layout().entityIdToStruct[
            entity
        ];
        ...
        values[1] = abi.encode(s.durationValue);
        values[2] = abi.encode(s.startCondition);
        values[3] = abi.encode(s.statusEffectType);
    }
    ...
    function getBytes(
        ...
        Layout memory s = CombatStatusEffectStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (
            ...
            s.startCondition,
            s.statusEffectType
        ) = abi.decode(value, (uint16, uint16, uint8, uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (
                ...
                s.startCondition,
                s.statusEffectType
            ) = abi.decode(values[i], (uint16, uint16, uint8, uint8));
    ...
    function remove(
        ...
        delete CombatStatusEffectStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete CombatStatusEffectStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
