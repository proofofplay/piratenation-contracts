...
uint256 constant ID = uint256(keccak256("game.piratenation.boolcomponent.v1"));

struct Layout {
    bool value;
}

library BoolComponentStorage {
...
/**
 * @title BoolComponent
 * @dev A generic boolean value of an entity
 */
contract BoolComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = BoolComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = BoolComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = BoolComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete BoolComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete BoolComponentStorage.layout().entityIdToStruct[entities[i]];
        }
