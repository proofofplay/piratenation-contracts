...
uint256 constant ID = uint256(
    keccak256("game.piratenation.validatortransformcomponent.v1")
);
...
library ValidatorTransformComponentStorage {
...
/**
 * @title ValidatorTransformComponent
 * @dev Component marks a transform as only available to validators
 */
contract ValidatorTransformComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether the transform is only available to validators
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether the transform is only available to validators
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = ValidatorTransformComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether the transform is only available to validators
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = ValidatorTransformComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ValidatorTransformComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete ValidatorTransformComponentStorage.layout().entityIdToStruct[
            entity
        ];
