...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gemcooldownmultipliercomponent.v1")
);
...
library GemCooldownMultiplierComponentStorage {
...
/**
 * @title GemCooldownMultiplierComponent
 * @dev Define the multiplier used for gem cooldown calculation
 */
contract GemCooldownMultiplierComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The multiplier used for gem cooldown calculation
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value The multiplier used for gem cooldown calculation
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = GemCooldownMultiplierComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value The multiplier used for gem cooldown calculation
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = GemCooldownMultiplierComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GemCooldownMultiplierComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete GemCooldownMultiplierComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete GemCooldownMultiplierComponentStorage
                .layout()
                .entityIdToStruct[entities[i]];
        }
