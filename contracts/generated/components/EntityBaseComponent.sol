...
uint256 constant ID = uint256(
    keccak256("game.piratenation.entitybasecomponent.v1")
);
...
library EntityBaseComponentStorage {
...
/**
 * @title EntityBaseComponent
 * @dev Component that holds the base reference entity for an entity
 */
contract EntityBaseComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Reference to the base entity
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = EntityBaseComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Reference to the base entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = EntityBaseComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = EntityBaseComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = EntityBaseComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete EntityBaseComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete EntityBaseComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
