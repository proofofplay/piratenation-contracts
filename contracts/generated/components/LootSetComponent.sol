...
library LootSetComponentStorage {
 ...
 */
contract LootSetComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     * @param lootId Id of the token to grant (ERC1155/LOOT TABLE/CALLBACK types only)
    ...
    function append(
        ...
        Layout storage s = LootSetComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function removeValueAtIndex(
        ...
        if (index != lastIndexInArray) {
            ...
            s.tokenContract[index] = s.tokenContract[lastIndexInArray];
            s.lootId[index] = s.lootId[lastIndexInArray];
    ...
    function getLayoutValue(
        ...
        value = LootSetComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     * @return lootId Id of the token to grant (ERC1155/LOOT TABLE/CALLBACK types only)
    ...
    {
        if (has(entity)) {
            Layout memory s = LootSetComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function remove(
        ...
        delete LootSetComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete LootSetComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
