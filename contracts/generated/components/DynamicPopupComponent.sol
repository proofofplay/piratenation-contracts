...
uint256 constant ID = uint256(
    keccak256("game.piratenation.dynamicpopupcomponent.v1")
);
...
library DynamicPopupComponentStorage {
...
/**
 * @title DynamicPopupComponent
 * @dev Config for a popup to be displayed dynamically to user
 */
contract DynamicPopupComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param popupType The type of popup to display in the game.
    ...
    function getLayoutValue(
        ...
        value = DynamicPopupComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return popupType The type of popup to display in the game.
    ...
    {
        if (has(entity)) {
            Layout memory s = DynamicPopupComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = DynamicPopupComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](6);
        values[0] = abi.encode(s.popupType);
        values[1] = abi.encode(s.popupTrigger);
        values[2] = abi.encode(s.priority);
        values[3] = abi.encode(s.maxCountPerLifetime);
        values[4] = abi.encode(s.maxCountPerDay);
        values[5] = abi.encode(s.maxCountPerSession);
    }
    ...
    function remove(
        ...
        delete DynamicPopupComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete DynamicPopupComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
