...
uint256 constant ID = uint256(
    keccak256("game.piratenation.generationcheckcomponent.v1")
);
...
library GenerationCheckComponentStorage {
...
/**
 * @title GenerationCheckComponent
 * @dev Define a generation check
 */
contract GenerationCheckComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The required generation trait
        keys[0] = "generation";
        values[0] = TypesLibrary.SchemaValue.UINT32;

        // Whether the generation check is required
        keys[1] = "required";
     ...
     *
     * @param entity Entity to get value for
     * @param generation The required generation trait
     * @param required Whether the generation check is required
     */
    function setValue(
        uint256 entity,
        uint32 generation,
        ...
        _setValue(entity, Layout(generation, required));
    }
    ...
    function getLayoutValue(
        ...
        value = GenerationCheckComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return generation The required generation trait
     * @return required Whether the generation check is required
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint32 generation, bool required) {
        if (has(entity)) {
            Layout memory s = GenerationCheckComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (generation, required) = abi.decode(
                _getEncodedValues(s),
                (uint32, bool)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GenerationCheckComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.generation);
        values[1] = abi.encode(s.required);
    }
    ...
    function setBytes(
        ...
        Layout memory s = GenerationCheckComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.generation, s.required) = abi.decode(value, (uint32, bool));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.generation, s.required) = abi.decode(values[i], (uint32, bool));
    ...
    function remove(
        ...
        delete GenerationCheckComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.generation, value.required));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.generation, value.required);
    }
}
