...
uint256 constant ID = uint256(
    keccak256("game.piratenation.milestonerulescomponent")
);
...
library MilestoneRulesComponentStorage {
...
/**
 * @title MilestoneRulesComponent
 * @dev Component for storing the rules and rewards of a set of milestones
 */
contract MilestoneRulesComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ids belonging to LootEntityArray components defining the rewards for each milestone
        keys[0] = "milestone_rewards";
        values[0] = TypesLibrary.SchemaValue.UINT256_ARRAY;

        // The time in seconds required for each milestone
        keys[1] = "milestone_times";
     ...
     *
     * @param entity Entity to get value for
     * @param milestoneRewards Entity ids belonging to LootEntityArray components defining the rewards for each milestone
     * @param milestoneTimes The time in seconds required for each milestone
     */
    function setValue(
        uint256 entity,
        ...
        _setValue(entity, Layout(milestoneRewards, milestoneTimes));
    }
    ...
    function append(
        ...
        Layout storage s = MilestoneRulesComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.milestoneRewards.length; i++) {
            s.milestoneRewards.push(values.milestoneRewards[i]);
            s.milestoneTimes.push(values.milestoneTimes[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.milestoneRewards, s.milestoneTimes));
    }
    ...
    function removeValueAtIndex(
        ...
        // Get the last index
        uint256 lastIndexInArray = s.milestoneRewards.length - 1;

        // Move the last value to the index to pop
        if (index != lastIndexInArray) {
            s.milestoneRewards[index] = s.milestoneRewards[lastIndexInArray];
            s.milestoneTimes[index] = s.milestoneTimes[lastIndexInArray];
        }

        // Pop the last value
        s.milestoneRewards.pop();
        s.milestoneTimes.pop();
    ...
    function getLayoutValue(
        ...
        value = MilestoneRulesComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return milestoneRewards Entity ids belonging to LootEntityArray components defining the rewards for each milestone
     * @return milestoneTimes The time in seconds required for each milestone
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = MilestoneRulesComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (milestoneRewards, milestoneTimes) = abi.decode(
                _getEncodedValues(s),
                (uint256[], uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        values = new bytes[](2);
        values[0] = abi.encode(s.milestoneRewards);
        values[1] = abi.encode(s.milestoneTimes);
    }
    ...
    function setBytes(
        ...
        Layout memory s = MilestoneRulesComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.milestoneRewards, s.milestoneTimes) = abi.decode(
            value,
            (uint256[], uint256[])
        );
    ...
    function remove(
        ...
        delete MilestoneRulesComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete MilestoneRulesComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.milestoneRewards = value.milestoneRewards;
        s.milestoneTimes = value.milestoneTimes;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.milestoneRewards, value.milestoneTimes)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.milestoneRewards, value.milestoneTimes);
    }
}
