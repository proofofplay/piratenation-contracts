...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tokenidcomponent.v3")
);
...
contract TokenIdComponent is BaseStorageComponentV2 {
     ...
     *
     * @param entity Entity to get value for
     * @param tokenAddress Contract address of the entity token
     * @param tokenId ID of the entity token
     */
    function setValue(
        uint256 entity,
        address tokenAddress,
        ...
        _setValue(entity, Layout(tokenAddress, tokenId));
    }
     ...
     *
     * @param entity Entity to get value for
     * @return tokenAddress Contract address of the entity token
     * @return tokenId ID of the entity token
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (address tokenAddress, uint256 tokenId) {
        if (has(entity)) {
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.tokenAddress, s.tokenId) = abi.decode(
                values[i],
                (address, uint256)
            );
