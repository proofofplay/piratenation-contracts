...
uint256 constant ID = uint256(
    keccak256("game.piratenation.leaderboardconfigcomponent.v1")
);
...
library LeaderboardConfigComponentStorage {
...
/**
 * @title LeaderboardConfigComponent
 * @dev The configuration for a leaderboard
 */
contract LeaderboardConfigComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(itemEntity, popupEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = LeaderboardConfigComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 itemEntity, uint256 popupEntity) {
        if (has(entity)) {
            Layout memory s = LeaderboardConfigComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (itemEntity, popupEntity) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = LeaderboardConfigComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = LeaderboardConfigComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.itemEntity, s.popupEntity) = abi.decode(value, (uint256, uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.itemEntity, s.popupEntity) = abi.decode(
                values[i],
                (uint256, uint256)
            );
    ...
    function remove(
        ...
        delete LeaderboardConfigComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.itemEntity, value.popupEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.itemEntity, value.popupEntity);
    }
}
