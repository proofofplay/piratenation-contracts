...
uint256 constant ID = uint256(
    keccak256("game.piratenation.starsigncomponent.v1")
);
...
library StarSignComponentStorage {
...
/**
 * @title StarSignComponent
 * @dev The star sign of an entity
 */
contract StarSignComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = StarSignComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = StarSignComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = StarSignComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = StarSignComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete StarSignComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete StarSignComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
