...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gemcostmultipliercomponent.v1")
);

struct Layout {
    uint128 resourceMultiplier;
    uint128 cooldownMultiplier;
}

library GemCostMultiplierComponentStorage {
...
/**
 * @title GemCostMultiplierComponent
 * @dev Define the cost multipliers used for gem calculations
 */
contract GemCostMultiplierComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The multiplier used for resource calculation
        keys[0] = "resource_multiplier";
        values[0] = TypesLibrary.SchemaValue.UINT128;

        // The multiplier used for cooldown calculation
        keys[1] = "cooldown_multiplier";
     ...
     *
     * @param entity Entity to get value for
     * @param resourceMultiplier The multiplier used for resource calculation
     * @param cooldownMultiplier The multiplier used for cooldown calculation
     */
    function setValue(
        uint256 entity,
        uint128 resourceMultiplier,
        ...
        _setValue(entity, Layout(resourceMultiplier, cooldownMultiplier));
    }
    ...
    function getLayoutValue(
        ...
        value = GemCostMultiplierComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return resourceMultiplier The multiplier used for resource calculation
     * @return cooldownMultiplier The multiplier used for cooldown calculation
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint128 resourceMultiplier, uint128 cooldownMultiplier)
    {
        if (has(entity)) {
            Layout memory s = GemCostMultiplierComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (resourceMultiplier, cooldownMultiplier) = abi.decode(
                _getEncodedValues(s),
                (uint128, uint128)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GemCostMultiplierComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.resourceMultiplier);
        values[1] = abi.encode(s.cooldownMultiplier);
    }
    ...
    function setBytes(
        ...
        Layout memory s = GemCostMultiplierComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.resourceMultiplier, s.cooldownMultiplier) = abi.decode(
            value,
            (uint128, uint128)
        );
    ...
    function remove(
        ...
        delete GemCostMultiplierComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.resourceMultiplier = value.resourceMultiplier;
        s.cooldownMultiplier = value.cooldownMultiplier;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.resourceMultiplier, value.cooldownMultiplier)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.resourceMultiplier, value.cooldownMultiplier);
    }
}
