...
uint256 constant ID = uint256(
    keccak256("game.piratenation.stringkeyvaluecomponent.v1")
);
...
library StringKeyValueComponentStorage {
...
/**
 * @title StringKeyValueComponent
 * @dev Key value pairs for strings
 */
contract StringKeyValueComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Key of the string parameter being set
        keys[0] = "key";
        values[0] = TypesLibrary.SchemaValue.STRING_ARRAY;

        // Value of the string parameter being set
        keys[1] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param key Key of the string parameter being set
     * @param value Value of the string parameter being set
     */
    function setValue(
        uint256 entity,
        string[] memory key,
        ...
        _setValue(entity, Layout(key, value));
    }
    ...
    function append(
        ...
        Layout storage s = StringKeyValueComponentStorage
            .layout()
        ...
        _emitSetBytes(entity, abi.encode(s.key, s.value));
    }
    ...
    function removeValueAtIndex(
        ...
        if (index != lastIndexInArray) {
            ...
            s.value[index] = s.value[lastIndexInArray];
        }
    ...
    function getLayoutValue(
        ...
        value = StringKeyValueComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return key Key of the string parameter being set
     * @return value Value of the string parameter being set
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = StringKeyValueComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (key, value) = abi.decode(
                _getEncodedValues(s),
                (string[], string[])
            );
        }
    ...
    function setBytes(
        ...
        Layout memory s = StringKeyValueComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.key, s.value) = abi.decode(value, (string[], string[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.key, s.value) = abi.decode(values[i], (string[], string[]));
    ...
    function remove(
        ...
        delete StringKeyValueComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete StringKeyValueComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.key, value.value));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.key, value.value);
    }
}
