...
uint256 constant ID = uint256(keccak256("game.piratenation.bancomponent"));

struct Layout {
    bool value;
}

library BanComponentStorage {
...
/**
 * @title BanComponent
 * @dev If a wallet address is banned from the system.
 */
contract BanComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ID (packed address) of the owner
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Entity ID (packed address) of the owner
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = BanComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Entity ID (packed address) of the owner
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = BanComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = BanComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = BanComponentStorage.layout().entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete BanComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete BanComponentStorage.layout().entityIdToStruct[entities[i]];
        }
