...
library CaptainComponentV2Storage {
 ...
 */
contract CaptainComponentV2 is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param nftEntity Entity of the Pirate NFT
     * @param lastSetCaptainTime Time of last captain set
     */
    function setValue(
        uint256 entity,
        uint256 nftEntity,
        ...
        _setValue(entity, Layout(nftEntity, lastSetCaptainTime));
    }
    ...
    function getLayoutValue(
        ...
        value = CaptainComponentV2Storage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return nftEntity Entity of the Pirate NFT
     * @return lastSetCaptainTime Time of last captain set
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = CaptainComponentV2Storage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CaptainComponentV2Storage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.nftEntity);
        values[1] = abi.encode(s.lastSetCaptainTime);
    }
    ...
    function getBytes(
        ...
        Layout memory s = CaptainComponentV2Storage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.nftEntity, s.lastSetCaptainTime) = abi.decode(
            value,
            (uint256, uint256)
        );
    ...
    function remove(
        ...
        delete CaptainComponentV2Storage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete CaptainComponentV2Storage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.nftEntity = value.nftEntity;
        s.lastSetCaptainTime = value.lastSetCaptainTime;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.nftEntity, value.lastSetCaptainTime)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.nftEntity, value.lastSetCaptainTime);
    }
}
