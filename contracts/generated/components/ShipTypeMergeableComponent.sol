...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shiptypemergeablecomponent.v1")
);
...
library ShipTypeMergeableComponentStorage {
...
/**
 * @title ShipTypeMergeableComponent
 * @dev Define if a ship type is allowed to merge
 */
contract ShipTypeMergeableComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether the ship type is allowed to merge
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether the ship type is allowed to merge
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = ShipTypeMergeableComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether the ship type is allowed to merge
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = ShipTypeMergeableComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShipTypeMergeableComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete ShipTypeMergeableComponentStorage.layout().entityIdToStruct[
            entity
        ];
