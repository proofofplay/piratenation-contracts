...
uint256 constant ID = uint256(keccak256("game.piratenation.tagscomponent.v1"));

struct Layout {
    string[] value;
}

library TagsComponentStorage {
...
/**
 * @title TagsComponent
 * @dev Adds string tags to a given entity
 */
contract TagsComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Tags for the given entity
     */
    function setValue(
        uint256 entity,
        string[] memory value
    ...
    function append(
        ...
        Layout storage s = TagsComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getLayoutValue(
        ...
        value = TagsComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Tags for the given entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (string[] memory value) {
        if (has(entity)) {
            Layout memory s = TagsComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function remove(
        ...
        delete TagsComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TagsComponentStorage.layout().entityIdToStruct[entities[i]];
        }
