...
contract CardDeckComponent is BaseStorageComponentV2 {
    ...
    function setValue(
        ...
        _setValue(entity, Layout(cardEntities));
    }
    ...
    function append(
        ...
        _emitSetBytes(entity, abi.encode(s.cardEntities));
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory cardEntities) {
        if (has(entity)) {
            Layout memory s = CardDeckComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (cardEntities) = abi.decode(_getEncodedValues(s), (uint256[]));
        }
    }
    ...
    function setBytes(
        ...
        (s.cardEntities) = abi.decode(value, (uint256[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.cardEntities) = abi.decode(values[i], (uint256[]));
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.cardEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.cardEntities);
    }
}
