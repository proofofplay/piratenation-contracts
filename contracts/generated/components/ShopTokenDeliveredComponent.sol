...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shoptokendeliveredcomponent.v1")
);
...
library ShopTokenDeliveredComponentStorage {
...
/**
 * @title ShopTokenDeliveredComponent
 * @dev Marks if a particular token purchase has been delivered
 */
contract ShopTokenDeliveredComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Has Been Delivered
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = ShopTokenDeliveredComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Has Been Delivered
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = ShopTokenDeliveredComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShopTokenDeliveredComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete ShopTokenDeliveredComponentStorage.layout().entityIdToStruct[
            entity
        ];
