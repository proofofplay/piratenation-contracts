...
uint256 constant ID = uint256(keccak256("game.piratenation.idcomponent.v3"));
 ...
 * @dev ID Component
 */
contract IdComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @return value ID of the entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (string memory value) {
        if (has(entity)) {
            Layout memory s = IdComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = IdComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete IdComponentStorage.layout().entityIdToStruct[entities[i]];
        }
