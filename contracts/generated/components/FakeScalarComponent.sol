...
library FakeScalarComponentStorage {
...
/**
 * @title FakeScalarComponent
 * @dev Test Component for Scalar Values
 */
contract FakeScalarComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = FakeScalarComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    {
        if (has(entity)) {
            Layout memory s = FakeScalarComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = FakeScalarComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = FakeScalarComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete FakeScalarComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete FakeScalarComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
