...
uint256 constant ID = uint256(keccak256("game.piratenation.xpcomponent.v1"));

struct Layout {
    uint256 value;
}

library XpComponentStorage {
...
/**
 * @title XpComponent
 * @dev The experience points of an entity
 */
contract XpComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = XpComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = XpComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = XpComponentStorage.layout().entityIdToStruct[entity];
    ...
    function getBytes(
        ...
        Layout memory s = XpComponentStorage.layout().entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete XpComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete XpComponentStorage.layout().entityIdToStruct[entities[i]];
        }
