...
uint256 constant ID = uint256(
    keccak256("game.piratenation.chainidcomponent.v1")
);
...
contract ChainIdComponent is BaseStorageComponentV2 {
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = ChainIdComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ChainIdComponentStorage.layout().entityIdToStruct[
            entity
        ];
