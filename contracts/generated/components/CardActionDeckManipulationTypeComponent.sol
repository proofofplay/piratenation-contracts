...
uint256 constant ID = uint256(
    keccak256("game.piratenation.cardactiondeckmanipulationtypecomponent.v1")
);
...
library CardActionDeckManipulationTypeComponentStorage {
...
/**
 * @title CardActionDeckManipulationTypeComponent
 * @dev Card Action Deck Manipulation Type Component is a card action component that defines data for type of card data that is being manipulated. For example, draw attack type cards
 */
contract CardActionDeckManipulationTypeComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(cardActionType));
    }
    ...
    function getLayoutValue(
        ...
        value = CardActionDeckManipulationTypeComponentStorage
            .layout()
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (uint8 cardActionType) {
        if (has(entity)) {
            Layout memory s = CardActionDeckManipulationTypeComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (cardActionType) = abi.decode(_getEncodedValues(s), (uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CardActionDeckManipulationTypeComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = CardActionDeckManipulationTypeComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.cardActionType) = abi.decode(value, (uint8));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.cardActionType) = abi.decode(values[i], (uint8));
    ...
    function remove(
        ...
        delete CardActionDeckManipulationTypeComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.cardActionType));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.cardActionType);
    }
}
