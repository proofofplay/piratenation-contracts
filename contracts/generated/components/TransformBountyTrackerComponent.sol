...
uint256 constant ID = uint256(
    keccak256("game.piratenation.transformbountytrackercomponent.v1")
);
...
library TransformBountyTrackerComponentStorage {
...
/**
 * @title TransformBountyTrackerComponent
 * @dev Track when an individual entity starts a bounty and the wallet that started it
 */
contract TransformBountyTrackerComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Tranform Instance Entity ID of the bounty started
     ...
     *
     * @param entity Entity to get value for
     * @param transformInstanceEntity Tranform Instance Entity ID of the bounty started
     * @param wallet Wallet used to set the bounty
     */
    function setValue(
        uint256 entity,
        uint256 transformInstanceEntity,
        ...
        _setValue(entity, Layout(transformInstanceEntity, wallet));
    }
    ...
    function getLayoutValue(
        ...
        value = TransformBountyTrackerComponentStorage
            .layout()
     ...
     *
     * @param entity Entity to get value for
     * @return transformInstanceEntity Tranform Instance Entity ID of the bounty started
     * @return wallet Wallet used to set the bounty
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 transformInstanceEntity, address wallet)
    {
        if (has(entity)) {
            Layout memory s = TransformBountyTrackerComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (transformInstanceEntity, wallet) = abi.decode(
                _getEncodedValues(s),
                (uint256, address)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TransformBountyTrackerComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function setBytes(
        ...
        Layout memory s = TransformBountyTrackerComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.transformInstanceEntity, s.wallet) = abi.decode(
            value,
            (uint256, address)
        );
    ...
    function remove(
        ...
        delete TransformBountyTrackerComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TransformBountyTrackerComponentStorage
                .layout()
                .entityIdToStruct[entities[i]];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.transformInstanceEntity, value.wallet)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.transformInstanceEntity, value.wallet);
    }
}
