...
uint256 constant ID = uint256(
    keccak256("game.piratenation.actionpointscomponent.v1")
);
...
library ActionPointsComponentStorage {
...
/**
 * @title ActionPointsComponent
 * @dev Holds action points value that is used for combat
 */
contract ActionPointsComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Action points value
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = ActionPointsComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Action points value
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = ActionPointsComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ActionPointsComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        delete ActionPointsComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ActionPointsComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
