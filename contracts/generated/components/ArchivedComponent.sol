...
uint256 constant ID = uint256(
    keccak256("game.piratenation.archivedcomponent.v1")
);
...
contract ArchivedComponent is BaseStorageComponentV2 {
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = ArchivedComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ArchivedComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = ArchivedComponentStorage.layout().entityIdToStruct[
            entity
        ];
