...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shoploottablecomponent.v1")
);
...
library ShopLootTableComponentStorage {
...
/**
 * @title ShopLootTableComponent
 * @dev Loot table to display for a shop SKU
 */
contract ShopLootTableComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ID of the loot table
        keys[0] = "loot_table_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param lootTableEntity Entity ID of the loot table
     */
    function setValue(
        uint256 entity,
        uint256 lootTableEntity
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(lootTableEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = ShopLootTableComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lootTableEntity Entity ID of the loot table
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 lootTableEntity) {
        if (has(entity)) {
            Layout memory s = ShopLootTableComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (lootTableEntity) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShopLootTableComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.lootTableEntity);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ShopLootTableComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.lootTableEntity) = abi.decode(value, (uint256));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.lootTableEntity) = abi.decode(values[i], (uint256));
    ...
    function remove(
        ...
        delete ShopLootTableComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ShopLootTableComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.lootTableEntity = value.lootTableEntity;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.lootTableEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.lootTableEntity);
    }
}
