...
uint256 constant ID = uint256(
    keccak256("game.piratenation.battlevalidationcomponent.v1")
);
...
library BattleValidationComponentStorage {
...
/**
 * @title BattleValidationComponent
 * @dev holds ipfs url to all data needed to validate a battle
 */
contract BattleValidationComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param ipfsUrl IPFS URL of the battle data
     */
    function setValue(
        uint256 entity,
        string calldata ipfsUrl
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(ipfsUrl));
    }
    ...
    function getLayoutValue(
        ...
        value = BattleValidationComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return ipfsUrl IPFS URL of the battle data
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (string memory ipfsUrl) {
        if (has(entity)) {
            Layout memory s = BattleValidationComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (ipfsUrl) = abi.decode(_getEncodedValues(s), (string));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = BattleValidationComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.ipfsUrl);
    }
    ...
    function setBytes(
        ...
        Layout memory s = BattleValidationComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.ipfsUrl) = abi.decode(value, (string));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.ipfsUrl) = abi.decode(values[i], (string));
    ...
    function remove(
        ...
        delete BattleValidationComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.ipfsUrl));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.ipfsUrl);
    }
}
