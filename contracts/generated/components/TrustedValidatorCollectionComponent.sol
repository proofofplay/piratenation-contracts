...
uint256 constant ID = uint256(
    keccak256("game.piratenation.trustedvalidatorcollectioncomponent.v1")
);

struct Layout {
    uint256[] trustedValidatorEntities;
}

library TrustedValidatorCollectionComponentStorage {
...
/**
 * @title TrustedValidatorCollectionComponent
 * @dev A collection of public trusted validators
 */
contract TrustedValidatorCollectionComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // An array of entities containing trusted validators
        keys[0] = "trusted_validator_entities";
     ...
     *
     * @param entity Entity to get value for
     * @param trustedValidatorEntities An array of entities containing trusted validators
     */
    function setValue(
        uint256 entity,
        uint256[] memory trustedValidatorEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(trustedValidatorEntities));
    }
    ...
    function append(
        ...
        Layout storage s = TrustedValidatorCollectionComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.trustedValidatorEntities.length; i++) {
            s.trustedValidatorEntities.push(values.trustedValidatorEntities[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.trustedValidatorEntities));
    }
    ...
    function removeValueAtIndex(
        ...
        // Get the last index
        uint256 lastIndexInArray = s.trustedValidatorEntities.length - 1;

        // Move the last value to the index to pop
        if (index != lastIndexInArray) {
            s.trustedValidatorEntities[index] = s.trustedValidatorEntities[
                lastIndexInArray
            ];
        }

        // Pop the last value
        s.trustedValidatorEntities.pop();
    ...
    function getLayoutValue(
        ...
        value = TrustedValidatorCollectionComponentStorage
            .layout()
            .entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return trustedValidatorEntities An array of entities containing trusted validators
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256[] memory trustedValidatorEntities)
    {
        if (has(entity)) {
            Layout memory s = TrustedValidatorCollectionComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (trustedValidatorEntities) = abi.decode(
                _getEncodedValues(s),
                (uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.trustedValidatorEntities);
    }
    ...
    function setBytes(
        ...
        Layout memory s = TrustedValidatorCollectionComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.trustedValidatorEntities) = abi.decode(value, (uint256[]));
    ...
    function batchSetBytes(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            (s.trustedValidatorEntities) = abi.decode(values[i], (uint256[]));
    ...
    function remove(
        ...
        delete TrustedValidatorCollectionComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.trustedValidatorEntities = value.trustedValidatorEntities;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.trustedValidatorEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.trustedValidatorEntities);
    }
}
