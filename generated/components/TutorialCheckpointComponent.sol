...
contract TutorialCheckpointComponent is BaseStorageComponentV2 {
     ...
     *
     * @param entity Entity to get value for
     * @param currentCheckpoint Current tutorial checkpoint this player is on
     * @param tutorialCompleted Whether the tutorial is completed
     */
    function setValue(
        uint256 entity,
        uint64 currentCheckpoint,
        ...
        _setValue(entity, Layout(currentCheckpoint, tutorialCompleted));
    }
     ...
     *
     * @param entity Entity to get value for
     * @return currentCheckpoint Current tutorial checkpoint this player is on
     * @return tutorialCompleted Whether the tutorial is completed
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint64 currentCheckpoint, bool tutorialCompleted)
    {
        if (has(entity)) {
            Layout memory s = TutorialCheckpointComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (currentCheckpoint, tutorialCompleted) = abi.decode(
                _getEncodedValues(s),
                (uint64, bool)
            );
        }
