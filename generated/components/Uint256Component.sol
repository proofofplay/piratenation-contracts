...
uint256 constant ID = uint256(
    keccak256("game.piratenation.uint256component.v1")
);
...
library Uint256ComponentStorage {
...
/**
 * @title Uint256Component
 * @dev A generic uint256 value of an entity
 */
contract Uint256Component is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = Uint256ComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = Uint256ComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = Uint256ComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete Uint256ComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete Uint256ComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
