...
uint256 constant ID = uint256(
    keccak256("game.piratenation.stringcomponent.v1")
);
...
library StringComponentStorage {
 ...
 * @dev A generic string value of an entity
 */
contract StringComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = StringComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = StringComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = StringComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete StringComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete StringComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
