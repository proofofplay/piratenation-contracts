...
uint256 constant ID = uint256(
    keccak256("game.piratenation.imagelistcomponent.v1")
);
...
library ImageListComponentStorage {
 ...
 * @dev List of images (and video) for the entity
 */
contract ImageListComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Optional width for the asset
        keys[3] = "width";
        values[3] = TypesLibrary.SchemaValue.UINT32_ARRAY;

        // Optional height for the asset
        keys[4] = "height";
        values[4] = TypesLibrary.SchemaValue.UINT32_ARRAY;
    }
     ...
     * @param width Optional width for the asset
     * @param height Optional height for the asset
     */
    function setValue(
        uint256 entity,
        string[] memory name,
        ...
        uint32[] memory width,
        uint32[] memory height
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(name, url, mimeType, width, height));
    }
    ...
    function append(
        ...
        Layout storage s = ImageListComponentStorage.layout().entityIdToStruct[
            entity
        ];
        for (uint256 i = 0; i < values.name.length; i++) {
            s.name.push(values.name[i]);
            s.url.push(values.url[i]);
            s.mimeType.push(values.mimeType[i]);
            s.width.push(values.width[i]);
            s.height.push(values.height[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(
            entity,
            abi.encode(s.name, s.url, s.mimeType, s.width, s.height)
        );
    }
    ...
    function removeValueAtIndexes(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            // Get the last index
            uint256 lastIndexInArray = s.name.length - 1;
            // Move the last value to the index to pop
            if (indexToRemove != lastIndexInArray) {
                s.name[indexToRemove] = s.name[lastIndexInArray];
                s.url[indexToRemove] = s.url[lastIndexInArray];
                s.mimeType[indexToRemove] = s.mimeType[lastIndexInArray];
                s.width[indexToRemove] = s.width[lastIndexInArray];
                s.height[indexToRemove] = s.height[lastIndexInArray];
            }
            // Pop the last value
            s.name.pop();
            s.url.pop();
    ...
    function getLayoutValue(
        ...
        value = ImageListComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     * @return url URL of the image
     * @return mimeType Mime type of the image
     * @return width Optional width for the asset
     * @return height Optional height for the asset
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = ImageListComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (name, url, mimeType, width, height) = abi.decode(
                _getEncodedValues(s),
                (string[], string[], string[], uint32[], uint32[])
            );
        }
    }
    ...
    function getByteValues(
        ...
        values = new bytes[](5);
        values[0] = abi.encode(s.name);
        values[1] = abi.encode(s.url);
        values[2] = abi.encode(s.mimeType);
        values[3] = abi.encode(s.width);
        values[4] = abi.encode(s.height);
    }
    ...
    function getBytes(
        ...
        Layout memory s = ImageListComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.name, s.url, s.mimeType, s.width, s.height) = abi.decode(
            value,
            (string[], string[], string[], uint32[], uint32[])
        );
    ...
    function remove(
        ...
        delete ImageListComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ImageListComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
