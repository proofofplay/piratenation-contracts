...
uint256 constant ID = uint256(keccak256("game.piratenation.levelcomponent.v1"));

struct Layout {
    uint256 value;
}

library LevelComponentStorage {
...
/**
 * @title LevelComponent
 * @dev Define the level of an entity
 */
contract LevelComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value The level of the entity
     */
    function setValue(
        uint256 entity,
        uint256 value
    ...
    function getLayoutValue(
        ...
        value = LevelComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value The level of the entity
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 value) {
        if (has(entity)) {
            Layout memory s = LevelComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = LevelComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete LevelComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete LevelComponentStorage.layout().entityIdToStruct[entities[i]];
        }
